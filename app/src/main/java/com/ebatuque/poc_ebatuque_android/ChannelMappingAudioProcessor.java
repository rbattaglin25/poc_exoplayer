...
/* package */ final class ChannelMappingAudioProcessor extends BaseAudioProcessor {
  ...
  private float[] outputChannelsVolume;
   ...
   */
  public void setChannelMap(@Nullable int[] outputChannels, float[] outputChannelsVolume) {
    pendingOutputChannels = outputChannels;
    this.outputChannelsVolume = outputChannelsVolume;
  }

  public float[] getOutputChannelsVolume() {
    return outputChannelsVolume;
  }

  public void initialize(int sampleRateHz, int channelCount, @C.Encoding int encoding)
    throws UnhandledAudioFormatException {
    AudioFormat audioFormat = new AudioFormat(sampleRateHz, channelCount, encoding);
  }
  ...
  public void queueInput(ByteBuffer inputBuffer) {
    ...
    while (position < limit) {
      for (int i = 0; i < 2; i++) { //i=0 -> left channel; i=1 -> right channel
        short sample = 0;
        for (int j = 0; j < 4; j++) {
          sample += (short) (inputBuffer.getShort(position + 4*j + 2*i) * outputChannelsVolume[2*j+i]);
        }
